...
import { FileText, Trash2, Pencil, Check, X, Calendar, Archive, ArchiveRestore } from "lucide-react";
...
export default function DoctorPrescriptionList() {
  ...
  const handleSave = async (id: string) => {
    if (!editDiagnosis || !editMedications) { alert("All fields are required"); return; }
    setSaving(true);
    try {
      ...
      if (res.ok) { setEditing(null); fetchPrescriptions(); }
      else alert("Failed to update prescription");
    } catch { alert("Error updating prescription"); }
  ...
  const handleArchive = async (id: string, archive: boolean) => {
    try {
      const res = await fetch(`/api/prescription/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ archivedByDoctor: archive }),
      });
  ...
  const active = prescriptions.filter((p) => !p.archivedByDoctor);
  const archived = prescriptions.filter((p) => p.archivedByDoctor);
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
              tab === "active" ? "bg-green-600 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
              tab === "archived" ? "bg-slate-600 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
            }`}
          >
        ...
        {displayed.length === 0 ? (
          <div className="text-center py-10">
            ...
            <p className="text-slate-400 font-semibold">
              {tab === "active" ? "No prescriptions issued yet" : "No archived prescriptions"}
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            {displayed.map((px) => (
              <div key={px.id} className={`rounded-xl border overflow-hidden ${
                tab === "archived" ? "border-slate-200 opacity-75" : "border-slate-200"
              }`}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {editing !== px.id && (
                      <>
                        ...
                        <button
                          onClick={() => handleArchive(px.id, !px.archivedByDoctor)}
                          className="p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors"
                          title={px.archivedByDoctor ? "Unarchive" : "Archive"}
                        >
                          {px.archivedByDoctor
